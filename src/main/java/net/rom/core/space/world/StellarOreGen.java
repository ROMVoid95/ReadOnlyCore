...
public class StellarOreGen implements IWorldGenerator {
	private int amountPerChunk;

	private int maxGenerateLevel;

	private int minGenerateLevel;

	private int amountPerVein;

	private IBlockState oreBlock;

	private IBlockState replaceBlock;

	private int dimId;

	public void generate(IBlockState oreBlock, int amountPerChunk, int minGenLevel, int maxGenLevel, int amountPerVein, IBlockState replace, int dim) {
	  ...
	  public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		  ...
		  private boolean generateOre(World par1World, Random par2Random, int par3, int par4, int par5) {
		    ...
		    for (int var19 = 0; var19 <= this.amountPerVein; var19++) {
		      ...
		      double var28 = (MathHelper.sin(var19 * 3.1415927F / this.amountPerVein) + 1.0F) * var26 + 1.0D;
		      double var30 = (MathHelper.sin(var19 * 3.1415927F / this.amountPerVein) + 1.0F) * var26 + 1.0D;
