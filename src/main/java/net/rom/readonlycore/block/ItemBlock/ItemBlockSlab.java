package net.rom.readonlycore.block.ItemBlock;
...
import net.rom.readonlycore.block.BlockSlabGeneric;

public class ItemBlockSlab extends ItemBlock {

	public ItemBlockSlab(BlockSlabGeneric slab) {
		super(slab);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (!itemstack.isEmpty() && player.canPlayerEdit(pos.offset(facing), facing, itemstack)) {
			...
			if (iblockstate.getBlock() == this.block) {
				BlockSlabGeneric.Part StarcraftSlab$Part = (BlockSlabGeneric.Part) iblockstate
						.getValue(BlockSlabGeneric.PART);

				if ((facing == EnumFacing.UP && StarcraftSlab$Part == BlockSlabGeneric.Part.BOTTOM
						|| facing == EnumFacing.DOWN && StarcraftSlab$Part == BlockSlabGeneric.Part.TOP)) {
	...
	public boolean canPlaceBlockOnSide(World worldIn, BlockPos pos, EnumFacing side, EntityPlayer player,
		...
		if (iblockstate.getBlock() == this.block) {
			boolean flag = iblockstate.getValue(BlockSlabGeneric.PART) == BlockSlabGeneric.Part.TOP;
		...
		return iblockstate1.getBlock() == this.block ? true
