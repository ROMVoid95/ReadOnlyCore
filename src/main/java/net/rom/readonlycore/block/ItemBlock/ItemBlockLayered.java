...
import net.rom.readonlycore.block.BlockLayered;
...
public class ItemBlockLayered extends ItemBlock {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!itemstack.isEmpty() && player.canPlayerEdit(pos, facing, itemstack)) {
			...
			if (block == this.block) {
				int i = ((Integer) iblockstate.getValue(BlockLayered.LAYERS)).intValue();

				if (i < 8) {
					IBlockState iblockstate1 = iblockstate.withProperty(BlockLayered.LAYERS, Integer.valueOf(i + 1));
					AxisAlignedBB axisalignedbb = iblockstate1.getCollisionBoundingBox(world, blockpos);

					if (axisalignedbb != Block.NULL_AABB && world.checkNoEntityCollision(axisalignedbb.offset(blockpos))
							&& world.setBlockState(blockpos, iblockstate1, 10)) {
	...
	public boolean canPlaceBlockOnSide(World world, BlockPos pos, EnumFacing side, EntityPlayer player,
		...
		return (state.getBlock() != this.block || ((Integer) state.getValue(BlockLayered.LAYERS)) > 7)
