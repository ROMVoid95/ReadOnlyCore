package net.rom.readonlycore.gui.menu;
...
public class MouseInput {
	...
	public static void init() {
		if (!init) {
			VRESIZE_CURSOR = loadCursor(new ResourceLocation("readonlycore", "cursor/vresize.png"), 32, 32, 16, 16);
			HRESIZE_CURSOR = loadCursor(new ResourceLocation("readonlycore", "cursor/hresize.png"), 32, 32, 16, 16);
			init = true;
		}
	}
	...
	private static Cursor loadCursor(ResourceLocation r, int width, int height, int xHotspot, int yHotspot) {
		try {
			BufferedImage i = CoreUtil.getImageResourceAsStream(r);
