...
import net.rom.readonlycore.client.render.Draw;
import net.rom.readonlycore.client.render.OpenGL;
...
public abstract class StormProvider implements Predicate<Entity>, IStormProvider {
	...
	public static void clientTickEvent(ClientTickEvent event) {
		if (CoreUtil.getMinecraft().world != null && CoreUtil.getMinecraft().world.provider instanceof IClimateProvider
				&& !CoreUtil.getMinecraft().isGamePaused()) {
			IClimateProvider climate = (IClimateProvider) CoreUtil.getMinecraft().world.provider;

			if (climate.getStormProvider() instanceof StormProvider) {
				StormProvider storm = (StormProvider) climate.getStormProvider();

				if (storm.isStormApplicableTo(CoreUtil.getMinecraft().world.provider)) {
					int s = storm.getStormSize();
					storm.updateStorm(CoreUtil.getMinecraft().world);
					...
					if (storm.isStormActive(CoreUtil.getMinecraft().world)) {
					...
					if (storm.isStormActive(CoreUtil.getMinecraft().world)) {
						...
						if (!CoreUtil.getMinecraft().gameSettings.fancyGraphics) {
							strength /= 2.0F;
						}

						if (strength != 0.0F) {
							Entity entity = CoreUtil.getMinecraft().getRenderViewEntity();
							...
							if (CoreUtil.getMinecraft().gameSettings.particleSetting == 1) {
								passes >>= 1;
							} else if (CoreUtil.getMinecraft().gameSettings.particleSetting == 2) {
								passes = 0;
							}
							...
							for (int i = 0; i < passes; ++i) {
								BlockPos pos1 = world.getPrecipitationHeight(
										blockpos.add(storm.random.nextInt(10) - storm.random.nextInt(10), 0,
												storm.random.nextInt(10) - storm.random.nextInt(10)));
	...
	public static void renderLast(RenderWorldLastEvent event) {
		if (CoreUtil.getMinecraft().world != null) {
			if (CoreUtil.getMinecraft().world.provider instanceof IClimateProvider) {
				IClimateProvider climate = (IClimateProvider) CoreUtil.getMinecraft().world.provider;
				climate.getStormProvider().renderStorm(event.getPartialTicks(), CoreUtil.getMinecraft().world,
			...
			for (int x = posX - stormDepth; x <= posX + stormDepth; ++x) {
				...
				if (isStormVisibleInBiome(biome) && renderStorm) {
					...
					if (minY != maxY) {
						...
						float vTravel = -(((CoreUtil.getMinecraft().world.getWorldTime() + (x * x) + x + (z * z) + z)
								& 31) + partialTicks) / getStormDownfallSpeed();
						float hTravel = (((CoreUtil.getMinecraft().world.getWorldTime() + (x * x) + x + (z * z) + z)
