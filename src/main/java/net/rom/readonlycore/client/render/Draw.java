...
import net.rom.readonlycore.client.Resources;
import net.rom.readonlycore.utils.CoreUtil;
import net.rom.readonlycore.utils.Worlds;
...
public class Draw {
	...
	public static void drawString(String text, int x, int y, int color, boolean shadow) {
		...
		text = ReadOnlyCore.translateUtil.translate(text);
		...
		if (shadow) {
			CoreUtil.getFontRenderer().drawStringWithShadow(text, x, y, color);
		}

		if (!shadow) {
			CoreUtil.getFontRenderer().drawString(text, x, y, color);
		}
	...
	public static void drawStringAlignCenter(String text, int x, int y, int w, int h, int color, boolean shadow) {
		drawString(text, x + (w - Draw.getStringRenderWidth(ReadOnlyCore.translateUtil.translate(text))) / 2,
				y + (h - 8) / 2, color, shadow);
	}
	...
	public static void drawStringAlignCenter(String text, int x, int y, int color, boolean shadow) {
		drawString(text, x - Draw.getStringRenderWidth(ReadOnlyCore.translateUtil.translate(text)) / 2, y, color,
				shadow);
	}
	...
	public static void drawStringAlignRight(String text, int x, int y, int color, boolean shadow) {
		drawString(text, x - Draw.getStringRenderWidth(ReadOnlyCore.translateUtil.translate(text)), y, color, shadow);
	}
	...
	public static int getStringRenderWidth(String s) {
		return CoreUtil.getFontRenderer().getStringWidth(TextFormatting.getTextWithoutFormattingCodes(s));
	}
	...
	public static void drawProgressBar(String label, int maxProgress, int curProgress, int posX, int posY, int barWidth,
		...
		OpenGL.pushMatrix();
		{
			...
			CoreUtil.getFontRenderer().drawStringWithShadow(label,
					posX + (barWidth / 2) - CoreUtil.getFontRenderer().getStringWidth(label)
							+ (CoreUtil.getFontRenderer().getStringWidth(label) / 2),
	...
	public static void drawEntity(int x, int y, int scale, float yaw, float pitch, Entity entity) {
		if (!CoreUtil.getMinecraft().world.isRemote) {
			...
			OpenGL.pushMatrix();
			{
				...
				CoreUtil.getMinecraft().getRenderManager().renderEntityStatic(entity, CoreUtil.getPartialTicks(), true);
	...
	public static void drawPlayerFace(EntityPlayer player, int x, int y, int width, int height) {
		if (player instanceof AbstractClientPlayer) {
			...
			CoreUtil.bindTexture(clientPlayer.getLocationSkin());
	...
	public static void drawParticle(int index, int x, int y, int width, int height) {
		...
		CoreUtil.bindTexture(Resources.PARTICLES);
	...
	public static void renderItem(ItemStack stack, int x, int y) {
		...
		CoreUtil.getMinecraft().getTextureManager().bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
		CoreUtil.getMinecraft().getTextureManager().getTexture(TextureMap.LOCATION_BLOCKS_TEXTURE).setBlurMipmap(false,
				false);
		...
		IBakedModel ibakedmodel = CoreUtil.getMinecraft().getRenderItem().getItemModelMesher().getItemModel(stack);
		ibakedmodel = ibakedmodel.getOverrides().handleItemState(ibakedmodel, stack, CoreUtil.getMinecraft().world,
				CoreUtil.getMinecraft().player);
		...
		CoreUtil.getMinecraft().getRenderItem().renderItem(stack, ibakedmodel);
		GlStateManager.disableAlpha();
		GlStateManager.disableRescaleNormal();
		GlStateManager.disableLighting();
		GlStateManager.popMatrix();
		CoreUtil.getMinecraft().getTextureManager().bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
		CoreUtil.getMinecraft().getTextureManager().getTexture(TextureMap.LOCATION_BLOCKS_TEXTURE)
	...
	public static void drawItem(ItemStack stack, int x, int y, int width, int height) {
		...
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		GlStateManager.disableLighting();
		CoreUtil.bindTexture(getResourceLocationFullPath(ibakedmodel.getParticleTexture()));
...
//	}
	...
	public static ResourceLocation getMissingTexture() {
		return getResourceLocationPartialPath(CoreUtil.getMinecraft().getTextureMapBlocks().getMissingSprite());
	}
	...
	public static ResourceLocation getResourceLocationFullPath(TextureAtlasSprite sprite) {
		if (sprite != null) {
			...
			return new ResourceLocation(r.getNamespace(), String.format("%s/%s%s",
					new Object[] { mc.getTextureMapBlocks().getBasePath(), r.getPath(), ".png" }));
		}
