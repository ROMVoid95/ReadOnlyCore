...
import net.rom.libs.autoreg.Register.RegisterBlock;
import net.rom.libs.autoreg.Register.RegisterBlockInit;
import net.rom.libs.autoreg.Register.RegisterItem;
import net.rom.libs.autoreg.Register.RegisterItemBlock;
import net.rom.libs.autoreg.Register.RegisterItemInit;
import net.rom.libs.autoreg.Register.RegisterRender;
import net.rom.libs.autoreg.Register.RegisterTESR;
import net.rom.libs.autoreg.Register.RegisterTileEntity;
...
public class Registrar {
	 ...
	 * <code>@Registry</code> annotation present. For use only in {@link ReadOnlyCore}
	 * 
	 * @param dataTable
	 *            The {@link ASMDataTable} to get the information from
	 */
	public static void addRegistryClasses(ASMDataTable dataTable) {
		ReadOnlyCore.logger.info("Searching for registrar classes");
		for (ASMData data : dataTable.getAll(Register.class.getName())) {
			try {
				...
				ReadOnlyCore.logger.info("Found registrar class: " + data.getClassName());
			} catch (Exception e) {
				ReadOnlyCore.logger.error("Unable add registrar class: " + data.getClassName() + "! An error occurred:");
				ReadOnlyCore.logger.catching(Level.ERROR, e);
			}
		}
		for (ASMData data : dataTable.getAll(RegisterTileEntity.class.getName())) {
			try {
				if (Class.forName(data.getClassName()).isInstance(TileEntity.class))
					...
					ReadOnlyCore.logger.warn("Found tile entity marker on non tile entity class: " + data.getClassName());
			} catch (Exception e) {
				ReadOnlyCore.logger.error("Unable to add tile entity: " + data.getClassName()
	...
	public static void onItemRegister(RegistryEvent.Register<Item> event) {
		ReadOnlyCore.logger.info("Searching for items to register");
				...
				for (Method method : clazz.getMethods()) {
					if (method.isAnnotationPresent(RegisterItemInit.class)) {
						try {
							...
							ReadOnlyCore.logger.error("Unable to initialise items using init method: " + method.getName()
				...
				for (Field field : clazz.getDeclaredFields()) {
					if (field.isAnnotationPresent(RegisterItem.class)) {
						try {
							...
							ReadOnlyCore.logger.error("Unable to register item: " + field.getName()
									+ "! The following error was thrown:");
							ReadOnlyCore.logger.catching(Level.ERROR, e);
						}
					}
					if (field.isAnnotationPresent(RegisterBlock.class)) {
						try {
							...
							if (details.registerItemBlock()) {
								...
								ItemBlock item = null;
								if (block instanceof ILibItemBlock) {
									ILibItemBlock customItemBlock = (ILibItemBlock) block;
							...
							ReadOnlyCore.logger.error("Unable to register item block: " + field.getName()
									+ "! The following error was thrown:");
							ReadOnlyCore.logger.catching(Level.ERROR, e);
						}
					}
		...
		ReadOnlyCore.logger.info("Successfully registered " + registeredItems + " items!");
		Loader.instance().setActiveModContainer(Loader.instance().getIndexedModList().get(ReadOnlyCore.MODID));
	}

	@SubscribeEvent
	public static void onBlockRegister(RegistryEvent.Register<Block> event) {
		ReadOnlyCore.logger.info("Searching for blocks and tiles to register");
				...
				for (Field field : clazz.getDeclaredFields()) {
					if (field.isAnnotationPresent(RegisterBlock.class)) {
						try {
							...
							ReadOnlyCore.logger.error("Unable to register block: " + field.getName()
									+ "! The following error was thrown:");
							ReadOnlyCore.logger.catching(Level.ERROR, e);
						}
					}
		...
		for (Pair<String, Class> tileData : tiles) {
			try {
				...
				ReadOnlyCore.logger.error(
						"Unable to register tile entity: " + tileData.getLeft() + "! The following error was thrown:");
				ReadOnlyCore.logger.catching(Level.ERROR, e);
			}
		}

		ReadOnlyCore.logger.info("Successfully registered " + registeredBlocks + " blocks!");
		ReadOnlyCore.logger.info("Successfully registered " + registeredTiles + " tiles!");
		Loader.instance().setActiveModContainer(Loader.instance().getIndexedModList().get(ReadOnlyCore.MODID));
	}
	...
	public static void onModelRegister(ModelRegistryEvent event) {
		ReadOnlyCore.logger.info("Searching for items to register renders for");
		int registeredItems = 0;
		int registeredBlocks = 0;
						...
						try {
							...
							if (field.get(null) instanceof Item) {
								...
								if (item != null) {
									...
									ReadOnlyCore.logger.error("Unable to register renders for item: " + field.getName()
								...
								if (block != null) {
									...
									ReadOnlyCore.logger.error("Unable to register renders for block: " + field.getName()
											+ "! The block cannot be null!");
								}
							}
						} catch (Exception e) {
							ReadOnlyCore.logger.error("Unable to register renders for: " + field.getName()
									+ "! The following error was thrown:");
							ReadOnlyCore.logger.catching(Level.ERROR, e);
						}
					}
		...
		for (Pair<Class, Class> tesrData : tesrs) {
			try {
				...
				ReadOnlyCore.logger.error("Unable to register TESR: " + tesrData.getRight().getName()
						+ "! The following error was thrown:");
				ReadOnlyCore.logger.catching(Level.ERROR, e);
			}
		}

		ReadOnlyCore.logger.info("Successfully registered renders for " + registeredItems + " items!");
		ReadOnlyCore.logger.info("Successfully registered renders for " + registeredBlocks + " item blocks!");
		ReadOnlyCore.logger.info("Successfully registered " + registeredTESRs + " TESRs!");
	}
	...
	public static <E extends Enum<E> & IStringSerializable> ResourceLocation[] generateModelVariants(String modid,
		...
		for (IStringSerializable variant : EnumerationHelper.getEnumValues(enumClass))
