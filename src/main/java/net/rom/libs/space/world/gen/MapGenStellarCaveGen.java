...
public class MapGenStellarCaveGen extends MapGenStellarBaseMeta {
	...
	public MapGenStellarCaveGen(Block block, int surfaceMeta, int subSurfaceMeta, int stoneMeta) {
		this.block = block;
	...
	protected void generateLargeCaveNode(long par1, int par3, int par4, ChunkPrimer primer, double par6, double par8,
			double par10) {
		this.generateCaveNode(par1, par3, par4, primer, par6, par8, par10, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F,
				0.0F, -1, -1, 0.5D);
	}

	protected void generateCaveNode(long par1, int par3, int par4, ChunkPrimer primer, double par6, double par8,
		...
		for (final boolean flag1 = random.nextInt(6) == 0; par15 < par16; ++par15) {
			final double d6 = 1.5D + MathHelper.sin(par15 * 3.1415927F / par16) * par12 * 1.0F;
			...
			if (!flag && par15 == k1 && par12 > 1.0F && par16 > 0) {
				this.generateCaveNode(random.nextLong(), par3, par4, primer, par6, par8, par10,
						random.nextFloat() * 0.5F + 0.5F, par13 - 3.1415927F / 2F, par14 / 3.0F, par15, par16, 1.0D);
				this.generateCaveNode(random.nextLong(), par3, par4, primer, par6, par8, par10,
						random.nextFloat() * 0.5F + 0.5F, par13 + 3.1415927F / 2F, par14 / 3.0F, par15, par16, 1.0D);
				return;
			}
									...
									if (xfactorSq + zfactorSq < 1.0D) {
										if (yfactor > -0.7D && xfactorSq + yfactorSq + zfactorSq < 1.0D) {
											...
											if (block == this.block) {
												...
												} else if (metadata == this.surfaceMeta && random
														.nextInt(MapGenStellarCaveGen.BREAK_THROUGH_CHANCE) == 0) {
													primer.setBlockState(localX, localY, localZ,
															Blocks.AIR.getDefaultState());
												}
											}
	...
	protected void recursiveGenerate(World par1World, int par2, int par3, int par4, int par5, ChunkPrimer primer) {
		...
		for (int var8 = 0; var8 < var7; ++var8) {
			...
			for (int var16 = 0; var16 < var15; ++var16) {
				final float var17 = this.rand.nextFloat() * 3.1415927F * 2F;
				...
				this.generateCaveNode(this.rand.nextLong(), par4, par5, primer, var9, var11, var13, var19, var17, var18,
						0, 0, 1.0D);
			}
		}
	}
